...
export function patch(oldVnode, vnode) {
  ...
  if (isRealElement) {
    ...
    if (oldChildren.length > 0 && newChildren.length === 0) {
      el.innerHTML = ''
      // 暴力写法直接清空；更好的处理是封装removeChildNodes方法：将子节点全部删掉，因为子节点可能包含组件
  ...
  while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
    ...
    if (!oldStartVnode) {
      ...
      if (moveIndex === undefined) { // 没有，将当前比对的新节点插入到老的头指针对用的节点前面
        ...
        // 将当前比对的老节点移动到老的头指针前面
        const moveVnode = oldChildren[moveIndex]// 从老的队列中找到可以被复用的这个节点
        // 复用：更新复用节点的属性，插入对应位置
...
function updateProperties(vnode, oldProps = {}) {
  ...
  const el = vnode.el // dom上的真实节点（上边复用老节点时已经赋值了）
  const newProps = vnode.data || {} // 拿到新的数据
